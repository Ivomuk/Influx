...
-- Adds 7d / 14d / 60d / 90d rolling windows, trend ratio features,
-- anti-gaming detection flags (timing manipulation, suspicious repayment jump, loan cycling),
...
-- -----------------------------------------------------------------------
-- Post-loan activity: last 7d vs prior 7d (existing logic, preserved)
-- -----------------------------------------------------------------------
...
-- -----------------------------------------------------------------------
-- Anti-gaming: inflow in last 2 days vs 30-day daily average
-- A spike > 2x the daily average immediately before evaluation is suspicious.
-- -----------------------------------------------------------------------
...
-- -----------------------------------------------------------------------
-- Anti-gaming: repayment ratio in last 7d vs prior 7d
-- A jump of more than 0.50 in a single week is suspicious.
-- -----------------------------------------------------------------------
repayment_jump AS (
    SELECT
        a.feature_dt,
...
-- -----------------------------------------------------------------------
-- Anti-gaming: loan cycling — disbursement within 3 days of a same-day repayment
-- Detects borrow → repay (from another lender) → borrow pattern.
-- -----------------------------------------------------------------------
loan_cycling AS (
    SELECT
        a.feature_dt,
...
-- -----------------------------------------------------------------------
-- Identity maturity: SIM age and activity breadth signals
-- -----------------------------------------------------------------------
...
-- -----------------------------------------------------------------------
-- Assemble all windows and compute derived features
-- -----------------------------------------------------------------------
combined AS (
    SELECT
        w30.feature_dt,
...
final_features AS (
    SELECT
        ...
        -- Repayment ratios across windows
        ...
        -- Repayment ratio trend: improvement or deterioration between 7d windows
        CASE
        ...
        -- Wallet activity across windows
        ...
        -- Wallet inflow trend: short-term vs long-term daily average
        CASE
        ...
        -- Borrowing counts and lender concentration
