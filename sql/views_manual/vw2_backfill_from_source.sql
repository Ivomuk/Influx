...
FROM (
    WITH raw_base AS (
        SELECT
            ...
            CAST(replace(CAST(txn_value AS VARCHAR), ',', '') AS DOUBLE) AS txn_value_num
        FROM analytics.momo_tran_loc_mapping_v2
        WHERE CAST(date_key AS VARCHAR) BETWEEN '20260324' AND '20260331'
          AND NOT (
                (service_name = 'Xtrafloat'   AND sub_service_name = 'Fee (Xtrafloat)')
             OR (service_name = 'Clinic Pesa' AND sub_service_name = 'Revenue Share (Clinic Pesa)')
             OR  service_name = 'Fees from Airtime reversals'
...
FROM (
    WITH raw_base AS (
        SELECT
            ...
            CAST(replace(CAST(txn_value AS VARCHAR), ',', '') AS DOUBLE) AS txn_value_num
        FROM analytics.momo_tran_loc_mapping_v2
        WHERE CAST(date_key AS VARCHAR) BETWEEN '20260401' AND '20260407'
          AND NOT (
                (service_name = 'Xtrafloat'   AND sub_service_name = 'Fee (Xtrafloat)')
             OR (service_name = 'Clinic Pesa' AND sub_service_name = 'Revenue Share (Clinic Pesa)')
             OR  service_name = 'Fees from Airtime reversals'
...
FROM (
    WITH raw_base AS (
        SELECT
            ...
            CAST(replace(CAST(txn_value AS VARCHAR), ',', '') AS DOUBLE) AS txn_value_num
        FROM analytics.momo_tran_loc_mapping_v2
        WHERE CAST(date_key AS VARCHAR) BETWEEN '20260408' AND '20260414'
          AND NOT (
                (service_name = 'Xtrafloat'   AND sub_service_name = 'Fee (Xtrafloat)')
             OR (service_name = 'Clinic Pesa' AND sub_service_name = 'Revenue Share (Clinic Pesa)')
             OR  service_name = 'Fees from Airtime reversals'
...
FROM (
    WITH raw_base AS (
        SELECT
            ...
            CAST(replace(CAST(txn_value AS VARCHAR), ',', '') AS DOUBLE) AS txn_value_num
        FROM analytics.momo_tran_loc_mapping_v2
        WHERE CAST(date_key AS VARCHAR) BETWEEN '20260415' AND '20260421'
          AND NOT (
                (service_name = 'Xtrafloat'   AND sub_service_name = 'Fee (Xtrafloat)')
             OR (service_name = 'Clinic Pesa' AND sub_service_name = 'Revenue Share (Clinic Pesa)')
             OR  service_name = 'Fees from Airtime reversals'
