# Dimension 3 — Feature Store Interface
# Provides hot / warm / cold path abstractions so the eligibility API can read
# pre-computed features at the appropriate freshness level.
#
# Hot path  : in-memory dict or Redis-equivalent — pre-computed features refreshed
#             every 15 minutes. Target read latency < 5 ms.
# Warm path : parquet/Delta table written by the near-real-time exposure refresh
#             job after every lender feedback event. Target freshness < 30 min.
# Cold path : full batch SQL output from vw3 + vw4 (daily or on-demand).
#             Used as fallback when hot/warm paths are stale or unavailable.
#
# In this v1 the store is backed by an in-memory dict keyed by subscriber_msisdn.
...
# ---------------------------------------------------------------------------
HOT_PATH_MAX_AGE_MINUTES = 15
WARM_PATH_MAX_AGE_MINUTES = 30
GRACEFUL_DEGRADATION_MAX_AGE_MINUTES = 60   # beyond this, do not serve

# ---------------------------------------------------------------------------
# Feature store state (in-memory; replace with Redis client in production)
# ---------------------------------------------------------------------------
...
def _age_minutes(written_at):
    if written_at is None:
        return float('inf')
    return (_now_utc() - written_at).total_seconds() / 60.0
...
def write_hot_features(features_df):
    """
    Writes a DataFrame of subscriber features to the hot path.
...
def write_warm_features(msisdn, updated_exposure_amt, lender_event_type):
    """
    Updates the warm path for a single subscriber immediately after a lender
    feedback event (disbursement or repayment). Only updates exposure fields
...
def read_features(msisdn, allow_stale=False, stale_threshold_minutes=None):
    """
    Reads the freshest available features for a subscriber.
    Priority: hot → warm → cold.
    Returns a tuple (features_dict, path_used, age_minutes, is_stale).
    Raises RuntimeError if no path has data within the graceful degradation window
    and allow_stale is False.
...
def read_features_batch(msisdn_list, allow_stale=False, stale_threshold_minutes=None):
    """
    Reads features for a list of MSISDNs.
    Returns a DataFrame with one row per subscriber plus path_used, age_minutes, is_stale columns.
    MSISDNs with no data are returned with all feature columns set to None
    and path_used = 'none'.
...
# ---------------------------------------------------------------------------
# Bulk load from SQL batch output (used in production bootstrap)
# ---------------------------------------------------------------------------

def load_from_batch_output(layer1_df, layer0_df):
    """
    Initialises the cold path from SQL batch DataFrames (vw3 + vw4 output).
